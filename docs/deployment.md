...
| **Admin login always fails** | `ADMIN_PASSWORD_HASH` set as a secret on the deployed Worker; the whole `pbkdf2$...` line was pasted; the hash came from the current `scripts/hash-password.mjs` (one from before the chained-rounds fix will never verify); not locked out (10 failures from one IP in 15 minutes); the `sessions` table exists |
